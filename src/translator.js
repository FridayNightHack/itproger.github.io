  ...
  const useTrnsltr = [
    {
      ...
      searchItems: null,
      langs: {
        translated: ['Главная', 'Качества', 'Индустрии', 'О нас', 'Связатья', 'РУ'],
      }
    },
    {
      id:'mainSection',
      semanticElems: ['h2', 'p', 'span', 'a'],
      searchItems: ['Professional', 'Machine Manufacturer', 'Duct',  'On the basis of integrity, to pursuit customer satisfaction.' , 'Continuous innovation, to help customers achieve their goals', 'Get a Quote', 'Contact Us'],
      langs: {
        translated: ['Профессиональный производитель',' машин', 'воздуховодных', 'Компания MasonCnc предлагает различные варианты линии по производству воздуховодов, здесь вы найдете то, что вам нужно.', 'Сделать заказ', 'Связаться с нами'],
      }
    },
    {
      id:'benefitsSection',
      ...
      searchItems: null,
      langs: {
        translated: ['100% собственность и управление MasonCnC', 'Основано в Нанкине'],
      }
    },
    {
      id:'benefitsList',
      ...
      langs: {
        translated: [
          ...
          'Производство и переработка','крупногабаритный расточно-фрезерный станок с ЧПУ, портальный обрабатывающий центр, станок плазменной резки с ЧПУ', 
          'Продажи на рынке', 
...
const useLang = () => {
  Array.from(useTrnsltr).map(parentBlock => {
    ...
    let section = document.getElementById(parentBlock?.id) ?? undefined
      if(section) {
        let semanticElems = section.querySelectorAll(parentBlock?.semanticElems) ?? undefined
        if(semanticElems) {
          Array.from(semanticElems).forEach(filtered => {
            for(let i = 0; i < filtered?.childNodes.length; i++) {
              if(filtered?.childNodes[i]?.nodeName === '#text' && filtered?.childNodes[i].data.indexOf('/n') != 0) {
                // console.log(filtered?.childNodes[i].data.trim());
                let text = filtered?.childNodes[i].data.trim();
                filtered.childNodes[i].data = parentBlock?.langs?.translated[counter] + " ";
                counter++;
              }
            }
          })
